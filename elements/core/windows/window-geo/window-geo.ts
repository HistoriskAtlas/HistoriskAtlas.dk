...
class WindowGeo extends polymer.Base implements polymer.Element {
    ...
    private insertEditorialText(text: string) {
        ...
        setTimeout(() => this.push('editorialContent.subContents', new HaSubContentText({ headline: '', text1: Common.html2rich(text), ordering: this.editorialContent.subContents.length, type: 0 }, this.editorialContent)), 100); //Timeout so Polymer has time to create the new HA-Content....
    }
