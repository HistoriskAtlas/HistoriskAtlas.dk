...
class WindowGeo extends polymer.Base implements polymer.Element {
    ...
    private insertEditorialText(text: string) {
        ...
        setTimeout(() => this.push('editorialContent.subContents', new HaSubContentText({ headline: '', text1: Common.html2rich(text), ordering: this.editorialContent.subContents.length, type: 0 }, this.editorialContent)), 1000); //Timeout so Polymer has time to create the new HA-Content....
    }
    ...
    shareFB() {
        Common.loadJS('facebook-jssdk', '//connect.facebook.net/da_DK/sdk.js', (e) => {
            (<any>window).FB.init({ appId: '876939902336614', xfbml: true, version: 'v11.0' });
            (<any>window).FB.ui({ method: 'share', href: this.geo.link, }, (response) => { });
        });
    }
    shareQR() {
